...
std::vector<std::pair<float, int>> HNSW::knn(const std::vector<float>& q, int k, DistFn dist) {
    return knn_ef(q, k, std::max(50, k), dist);
}

std::vector<std::pair<float, int>> HNSW::knn_ef(
