...
function drawFrame() {
  ...
  ctx.fillStyle=COL.bg; ctx.fillRect(0,0,sc.width,sc.height);
  ...
  ctx.fillStyle=COL.text; ctx.font='11px Fira Code,monospace';
  ...
  ctx.fillStyle=COL.muted; ctx.font='12px Fira Code,monospace';
  ...
  if (queryPt && hitIds.size>0) {
    ...
    for (const pt of pcaPoints) {
      ...
      ctx.strokeStyle=COL.accent; ctx.lineWidth=1; ctx.setLineDash([4,4]);
  ...
  if (queryPt) {
    ...
    ctx.fillStyle=COL.muted;ctx.font='10px Fira Code,monospace';ctx.fillText('query',qx+16,qy+4);
  }
  if (!pcaPoints.length) {
    ctx.fillStyle=COL.text;ctx.font='13px Fira Code,monospace';ctx.textAlign='center';
...
function drawVecChart(emb) {
  ...
  for (let i=0;i<DIMS;i++){
    const h=emb[i]*58,x=2+i*bw,col=getDimCol(i);
  ...
  [['CS',0],['MATH',4],['FOOD',8],['SPORT',12]].forEach(([lbl,gi],i)=>{
    vx.fillStyle=getDimCol(i*4); vx.globalAlpha=0.5;vx.fillText(lbl,2+(gi+1.5)*bw,74);
  });
  vx.textAlign='left';
}
...
async function askAI() {
  ...
  try {
    ...
    if (!d.error) { /* already appended above */ }
    else history.appendChild(aDiv);
